...
function Datatable() {
  ...
  const calculateNewTotalExpectedCount = () => {
    let newTotalExpectedCount = 0;
    departmentData.forEach((department) => {
      newTotalExpectedCount += parseInt(department.expectedCount || 0); // Ensure it handles undefined or null values
    });
    return newTotalExpectedCount;
  };
  ...
  const handleOpenModal = (department) => {
    ...
    setNewExpectedCount(department.expectedCount.toString());
    setOpenModal(true);
  };
  ...
  const handleSubmitExpCount = async () => {
    if (!selectedDepartment || !newExpectedCount) {
      alert('Please enter a valid expected count.');
      return;
    }
    if (isNaN(newExpectedCount) || parseInt(newExpectedCount) < 0) {
    ...
    try {
      ...
      const response = await axios.put(
        `${url}/update-expected-count`,
        {
          deptId: selectedDepartment.DeptID,
          subDeptId: selectedDepartment.SubDeptID || null,
          newExpectedCount: parseInt(newExpectedCount),
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.status === 200) {
        ...
        alert('Failed to update expected count.');
      }
    } catch (error) {
      console.error('Error updating expected count:', error);
      alert('Error updating expected count.');
    }
  };
  ...
  return (
    <Box>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  >
                    ...
                    <TableCell>{calculateNewTotalExpectedCount()}</TableCell>
      ...
      {/* Modal for setting expected count */}
      <Dialog open={openModal} onClose={handleCloseModal}>
        <DialogTitle>
          Set Expected Count for {selectedDepartment?.DeptName}{' '}
          {selectedDepartment?.SubDeptName ? ` - ${selectedDepartment.SubDeptName}` : ''}
        </DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
            label="New Expected Count"
            type="number"
            fullWidth
        ...
        <DialogActions>
          ...
          <Button onClick={handleSubmitExpCount}>Submit</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
