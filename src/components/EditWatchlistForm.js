...
function EditWatchlistForm({ username }) {
  ...
  useEffect(() => {
    const fetchOwnerSuggestions = async () => {
      if (watchlist.WatchListPrimaryOwnerName.trim() !== "") {
        try {
          const response = await axios.get(`${url}/employees?name=${watchlist.WatchListPrimaryOwnerName}`);
          setOwnerSuggestions(response.data.map((item) => ({ EmployeeID: item.EmpID, EmployeeName: item.EmpName, EmployeeEmail: item.EmpEmail })));
        } catch (error) {
  ...
  const fetchEmployeeSuggestions = async (inputValue) => {
    try {
      ...
      setEmployeeSuggestions(response.data.map((item) => ({ EmployeeID: item.EmpID, EmployeeName: item.EmpName, EmployeeEmail: item.EmpEmail })));
    } catch (error) {
...
const validateForm = () => {
  ...
  if (!watchlist.WatchListDescription.trim()) errors.watchlistDescription = "Watchlist description is required";
  if (!watchlist.WatchListPrimaryOwnerName) errors.watchlistOwner = "Please select the owner";
  if (!ownerSuggestions.some(owner => owner.EmployeeName === watchlist.WatchListPrimaryOwnerName)) errors.watchlistOwner = "Invalid owner name";
  ...
  return (
    <>
      <Box px={2} pt={2} textAlign="" minHeight="calc(100vh - 128px)">
        <Typography variant="h5" gutterBottom>Edit Watchlist</Typography>
        <form onSubmit={handleSubmit}>
          <Box display="flex" flexDirection="column" width="100%" margin="0 auto" mt={2}>
            <Box mb={2} display="flex" alignItems="center">
              <Typography variant="subtitle1" style={{ marginRight: "10px", width: "250px" }}>Watchlist Name:</Typography>
              <TextField
            ...
            <Box mb={2} display="flex" alignItems="center">
              <Typography variant="subtitle1" style={{ marginRight: "10px", width: "250px" }}>Watchlist Description:</Typography>
              <TextField
            ...
            <Box mb={2} display="flex" alignItems="center">
              <Typography variant="subtitle1" style={{ marginRight: "10px", width: "250px" }}>Watchlist Owner:</Typography>
              <Autocomplete
                style={{ width: "70%" }}
                fullWidth
                value={watchlist.WatchListPrimaryOwnerName}
                onChange={(event, value) => setWatchlist(prevState => ({
                  ...prevState,
                  WatchListPrimaryOwnerName: value || ""
                }))}
                inputValue={watchlist.WatchListPrimaryOwnerName}
                onInputChange={(event, newInputValue) => setWatchlist(prevState => ({
                  ...prevState,
            ...
            <Box mb={2} display="flex" alignItems="center">
              <Typography variant="subtitle1" style={{ marginRight: "10px", width: "250px" }}>Employees in Watchlist:</Typography>
              <Autocomplete
                style={{ width: "70%" }}
                multiple
                ...
                renderTags={(value, getTagProps) => value.map((option, index) => (
                  <Chip variant="outlined" label={option.EmployeeName} {...getTagProps({ index })} />
                ))}
                renderInput={(params) => (
                  <TextField
                    {...params}
            ...
            <Box display="flex" justifyContent="center" mt={2}>
              <Button variant="contained" color="primary" type="submit" style={{ marginRight: "10px" }}>Update</Button>
              <Button variant="contained" onClick={() => navigate('/watchlist')}>Cancel</Button>
            </Box>
            </Box>
    </form>
  </Box>
